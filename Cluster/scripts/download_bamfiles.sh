...
grep -f ../metadata/samples.txt ../metadata/Simons_meta_ENArun.txt | cut -f 5 | grep -f - ../metadata/ena.ftp.pointers.txt | cut -f 4,5 | awk '{print "mkdir -p " $1 " ; wget -P " $1 " ftp://" $2}' | bash